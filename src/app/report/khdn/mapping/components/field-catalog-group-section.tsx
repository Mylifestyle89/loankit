  ...
  return (
    <div className={isCollapsed ? "hidden" : "space-y-3"}>
      ...
      <div className="flex items-center gap-2.5 pt-1 pb-0.5">
        <span className="shrink-0 text-[13px] font-bold tracking-wide text-brand-600 dark:text-brand-400">
          {node.parent}
        </span>
        <div className="flex-1 border-t border-brand-200/60 dark:border-brand-500/25" />
      </div>
      ...
      {node.children.map((child) => (
        <div key={child.fullPath} className="space-y-2">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200/60 dark:border-white/[0.07] bg-slate-50/70 dark:bg-white/[0.04] p-2">
            <span className="rounded-full bg-brand-100 dark:bg-brand-500/10 px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-brand-600 dark:text-brand-400">
              ...
              <button
                ...
                className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-[11px] font-medium transition-colors ${
                  child.fields.some((f) => f.is_repeater)
                    ? "border-brand-200 bg-brand-100 text-brand-700 hover:bg-brand-200 dark:border-brand-500/30 dark:bg-brand-500/10 dark:text-brand-300 dark:hover:bg-brand-500/20"
                    : "border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-white/[0.05] text-zinc-600 dark:text-slate-300 hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700 dark:hover:bg-brand-500/10"
                }`}
          ...
          {/* Repeater group */}
          {child.fields.some((f) => f.is_repeater) ? (
            <div className="space-y-4 rounded-lg border border-brand-200/60 bg-brand-50/30 p-4">
              {((Array.isArray(values[child.fullPath]) ? values[child.fullPath] : []) as Record<string, unknown>[]).map(
                (item, index) => (
                  <div
                    key={index}
                    className="rounded-xl border border-brand-200/60 bg-white dark:bg-[#141414]/90 shadow-sm overflow-hidden"
                  >
                    <div className="flex items-center justify-between border-b border-brand-100 bg-brand-100 dark:bg-brand-500/10 px-4 py-2">
                      <span className="text-xs font-semibold text-brand-700">Bản ghi #{index + 1}</span>
                      <button
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-brand-300 bg-brand-50/50 px-4 py-2 text-sm font-medium text-brand-600 transition-colors hover:border-brand-400 hover:bg-brand-100"
              >
