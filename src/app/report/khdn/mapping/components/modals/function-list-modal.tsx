  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-hidden rounded-xl bg-white dark:bg-[#141414]/90 shadow-2xl flex flex-col">
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-white/[0.07] bg-amber-50/30 dark:bg-white/[0.04] px-4 py-3">
          <h2 className="text-lg font-semibold text-zinc-800 dark:text-slate-100">Danh sách hàm tính toán</h2>
          <button
            type="button"
            onClick={onClose}
            className="rounded p-1.5 text-zinc-500 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-white/[0.07] hover:text-zinc-800 dark:hover:text-slate-100"
            aria-label="Đóng"
          >
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          <p className="text-sm text-amber-700 dark:text-slate-300">
            ...
            Trong biểu thức có thể gõ mã trường (ví dụ <code className="rounded bg-amber-50/50 dark:bg-white/[0.06] px-1">custom.doanh_thu.doanh_thu</code>) hoặc tên Alias: nếu Alias có khoảng trắng (ví dụ &quot;Doanh thu thuần&quot;) thì trong công thức gõ bằng <strong>dấu gạch dưới</strong> (ví dụ <code className="rounded bg-amber-50/50 dark:bg-white/[0.06] px-1">Doanh_thu_thuần</code>). Không nên đặt hai Alias trùng nhau (sau khi chuẩn hóa) để tránh lỗi công thức.
          </p>
          ...
          <div>
            ...
            <ul className="space-y-4">
              {FUNCTIONS.filter((f) => ["sum", "average", "min", "max"].includes(f.nameEn)).map((f) => (
                <li key={f.nameEn} className="rounded-lg border border-zinc-100 dark:border-white/[0.07] bg-amber-50/50 dark:bg-white/[0.04] p-3">
                  <div className="font-medium text-amber-800 dark:text-slate-200">{f.nameVi}</div>
                  <div className="mt-1 text-xs font-sans text-zinc-500 dark:text-slate-300">{f.syntax}</div>
                  <p className="mt-1 text-sm text-amber-700 dark:text-slate-300">{f.desc}</p>
                  {f.example && (
          ...
          <div>
            ...
            <div className="rounded-lg border border-zinc-100 dark:border-white/[0.07] bg-amber-50/50 dark:bg-white/[0.04] p-3">
