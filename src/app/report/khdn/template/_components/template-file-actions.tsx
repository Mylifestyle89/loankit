    ...
    <div className="flex items-center gap-1">
      ...
      {onRegisterTemplate && !showRegisterInput && (
        <button type="button" disabled={registering} title="Đăng ký mẫu" onClick={openRegisterInput}
          className="rounded-md bg-amber-600 px-2 py-1 text-xs font-medium text-white hover:bg-amber-700 disabled:opacity-50 transition-all">
      ...
      {showRegisterInput && (
        <div className="flex items-center gap-1">
          <input
            ...
            className="w-32 rounded-md border border-zinc-300 dark:border-white/10 bg-white dark:bg-[#1a1a1a] px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-amber-500"
          />
          <button type="button" onClick={() => void submitRegister()} disabled={registering || !registerName.trim()} title="Xác nhận"
            className="rounded-md bg-amber-600 p-1 text-white hover:bg-amber-700 disabled:opacity-50 transition-all">
