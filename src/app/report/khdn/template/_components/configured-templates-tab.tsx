    ...
    <div className="rounded-2xl border border-zinc-200 dark:border-white/[0.07] bg-white dark:bg-[#161616] p-4 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        <select value={activeTemplateId} onChange={(e) => onActiveTemplateChange(e.target.value)} aria-label={t("template.editor.title")}
          className="rounded-lg border border-zinc-200 dark:border-white/[0.09] bg-white dark:bg-[#1a1a1a] dark:text-slate-100 px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40">
        ...
        <button type="button" onClick={onOpenDocx} disabled={!profileDocxPath}
          className="rounded-lg border border-zinc-200 dark:border-white/[0.09] bg-white dark:bg-[#1a1a1a] px-4 py-2 text-sm shadow-sm hover:border-primary-200 dark:hover:border-primary-500/20 disabled:opacity-50">
          {t("template.editor.openDocx")}
        </button>
        <button type="button" onClick={onOpenEditor} disabled={!profileDocxPath || openingEditor}
          className="rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium text-white shadow-sm shadow-primary-500/25 hover:brightness-110 disabled:opacity-50">
