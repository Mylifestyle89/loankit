    ...
    <div className="rounded-2xl border border-zinc-200 dark:border-white/[0.07] bg-white dark:bg-[#161616] p-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {onUploadValidate && (
            <button type="button" onClick={onUploadValidate} className="rounded-lg bg-brand-500 px-3 py-1.5 text-xs font-medium text-white shadow-sm shadow-brand-500/25 hover:brightness-110 transition-all">
              Upload &amp; Validate
            </button>
          )}
          <button type="button" onClick={() => void fetchTree()} className="rounded-lg border border-zinc-200 dark:border-white/[0.09] bg-white dark:bg-[#1a1a1a] px-3 py-1.5 text-xs shadow-sm hover:border-brand-200 dark:hover:border-brand-500/20 transition-all">
            Làm mới
          </button>
        </div>
      </div>
  ...
  return (
    <>
      <button
        ...
        className="flex w-full items-center gap-2 py-2 pr-3 text-left text-sm font-medium hover:bg-brand-50/60 dark:hover:bg-brand-900/20 transition-colors"
      >
