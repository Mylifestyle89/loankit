    ...
    <div className="rounded-xl border border-zinc-200 dark:border-white/[0.07] bg-white dark:bg-[#161616] p-4 shadow-sm space-y-3">
      <div className="flex items-center justify-between">
        ...
        {loanPlan ? (
          ...
          <button
            type="button"
            onClick={onAssign}
            className="rounded-lg bg-brand-500 px-3 py-1 text-xs font-medium text-white hover:bg-brand-600 transition-all flex items-center gap-1"
          >
      ...
      {loanPlan ? (
        <Link
          ...
          className="flex items-center gap-3 rounded-lg border border-brand-300 dark:border-brand-500/30 bg-brand-50/50 dark:bg-brand-500/5 px-3 py-2.5 transition-colors hover:bg-brand-100/50 dark:hover:bg-brand-500/10"
        >
          <FileText className="h-4 w-4 shrink-0 text-brand-500 dark:text-brand-400" />
          <span className="flex-1 min-w-0 truncate text-sm font-medium text-brand-700 dark:text-brand-300">
            {loanPlan.name}
          </span>
          <ExternalLink className="h-3.5 w-3.5 shrink-0 text-brand-400 dark:text-brand-500" />
        </Link>
      ) : (
