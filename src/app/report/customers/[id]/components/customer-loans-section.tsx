...
const statusColors: Record<string, string> = {
  ...
  pending: "bg-primary-50 text-primary-600 dark:bg-primary-500/10 dark:text-primary-400",
...
function LoanRow({ loan }: { loan: Loan }) {
  ...
  return (
    <div className={`rounded-xl border-2 overflow-hidden transition-all duration-200 ${
      expanded
        ? "border-primary-300 dark:border-primary-500/30 shadow-md shadow-primary-500/5"
      ...
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3.5 text-left cursor-pointer transition-colors ${
          expanded ? "bg-primary-50/50 dark:bg-primary-500/5" : "hover:bg-zinc-50 dark:hover:bg-white/[0.03]"
        }`}
      >
        {expanded ? <ChevronDown className="h-4 w-4 text-primary-500 shrink-0" /> : <ChevronRight className="h-4 w-4 text-zinc-400 shrink-0" />}
        ...
        <div className="border-t border-zinc-100 dark:border-white/[0.05] bg-zinc-50/50 dark:bg-white/[0.02] px-4 py-3 space-y-3">
          ...
          <div className="flex items-center gap-4 pt-1">
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium text-white shadow-sm shadow-primary-500/25 hover:brightness-110 transition-all cursor-pointer"
            >
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={!customerId}
          className="rounded-lg px-4 py-2 text-sm font-medium inline-flex items-center gap-1.5 bg-primary-500 text-white shadow-sm shadow-primary-500/25 hover:brightness-110 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
        >
