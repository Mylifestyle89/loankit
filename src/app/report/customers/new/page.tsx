    ...
    <section className="space-y-5">
      ...
      >
        ...
        <div className="flex gap-2">
          {([
            { key: "corporate" as const, label: "Doanh nghiệp" },
            { key: "individual" as const, label: "Cá nhân" },
          ]).map((opt) => (
            <button
              key={opt.key}
              type="button"
              ...
              className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors duration-150 ${
                customerType === opt.key
                  ? "bg-violet-100 text-violet-700 ring-1 ring-violet-500/30 dark:bg-violet-500/15 dark:text-violet-400"
                  : "bg-zinc-100 text-zinc-500 dark:bg-white/[0.06] dark:text-slate-400 hover:bg-zinc-200 dark:hover:bg-white/[0.1]"
              }`}
            >
              {opt.label}
            </button>
