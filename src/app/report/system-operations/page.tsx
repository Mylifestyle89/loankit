...
import { Settings, Database, UserCog, Package, FileSearch } from "lucide-react";
import { useLanguage } from "@/components/language-provider";
import { authClient } from "@/lib/auth-client";
import { DataManagementTab } from "./data-management-tab";
import { LoanProductsTab } from "./loan-products-tab";
import { TemplateSearchReplaceTab } from "./template-search-replace-tab";
...
export default function SystemOperationsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"data" | "products" | "templates" | "account" | "admin">("data");

  const tabs = [
    { key: "data" as const, label: t("systemOps.title"), icon: Database },
    { key: "products" as const, label: "Sản phẩm tín dụng", icon: Package },
    { key: "templates" as const, label: "Tìm & Thay thế mẫu", icon: FileSearch },
    ...
    <section className="space-y-5 max-w-6xl">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-amber-100 dark:border-amber-500/10 bg-gradient-to-br from-amber-50 via-white to-orange-50 dark:from-amber-950/30 dark:via-[#141414] dark:to-orange-950/20 p-5">
        ...
        <div className="relative">
          <h1 className="text-xl font-bold tracking-tight bg-gradient-to-r from-amber-700 to-orange-500 dark:from-amber-400 dark:to-orange-400 bg-clip-text text-transparent">
            {t("nav.systemOps")}
          </h1>
        ...
        {/* Tab switcher */}
        <div className="relative mt-4 flex gap-1 rounded-lg bg-white/60 dark:bg-white/[0.04] p-1 border border-amber-100/60 dark:border-white/[0.06] w-fit">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)}
                className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-all duration-150 ${
                  isActive
                    ? "bg-white dark:bg-white/[0.08] text-amber-700 dark:text-amber-400 shadow-sm"
