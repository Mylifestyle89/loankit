          ...
          <>
            ...
            <div className="space-y-2 rounded-lg border border-slate-200/60 dark:border-white/[0.07] bg-white/70 dark:bg-[#141414]/90 p-3">
              ...
              <div className="space-y-1">
                ...
                <select value={importMode} onChange={(e) => setImportMode(e.target.value as "append" | "overwrite")} aria-label={t("mapping.import.modeLabel")} className="w-full rounded-lg border border-slate-200/80 dark:border-white/[0.09] bg-white dark:bg-white/[0.05] dark:text-slate-100 px-3 py-2 text-sm font-medium focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-400/30 dark:focus:ring-violet-400/20">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.2 }} className="fixed inset-0 z-[111] flex items-center justify-center p-4">
            <div className="w-full max-w-md rounded-xl border border-slate-200 dark:border-white/[0.07] bg-white dark:bg-[#141414]/90 shadow-2xl">
              ...
              <div className="border-t border-slate-200 dark:border-white/[0.07] px-6 py-3 flex items-center justify-end gap-2">
                ...
                <button type="button" onClick={handleConfirmImportName} disabled={!importNameInput.trim()} className="rounded-lg bg-gradient-to-r from-violet-600 to-fuchsia-600 px-4 py-2 text-sm font-medium text-white transition-all hover:brightness-110 shadow-sm shadow-violet-500/25 disabled:opacity-60">Xác nhận</button>
              </div>
            </div>
          </motion.div>
        </>,
