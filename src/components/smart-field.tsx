  ...
  return (
    <div className="relative group/smart">
      <div className="flex items-center gap-1">
        ...
        >
          {hasOptions
            ? <ListPlus className="h-3.5 w-3.5 text-brand-500" />
        ...
        <div className="absolute z-50 top-full mt-1 right-0 w-64 rounded-xl border border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#1a1a1a] shadow-lg p-3 space-y-2">
          ...
          <div className="flex gap-1.5">
            <input
              ...
              className="flex-1 rounded-md border border-zinc-200 dark:border-white/[0.10] bg-white dark:bg-white/[0.04] px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-brand-500/40"
            />
            <button
              type="button"
              onClick={handleAdd}
              disabled={adding || !newLabel.trim()}
              className="rounded-md bg-brand-500 px-2 py-1 text-xs font-medium text-white disabled:opacity-40 hover:brightness-110 transition-all"
            >
              {adding ? "..." : "Thêm"}
            </button>
          </div>
