      ...
      <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-[#141414]/90 shadow-xl">
          ...
          <div className="flex items-center gap-2">
            {customerId && (
              <button type="button" onClick={handleSyncFromPlan} disabled={syncing}
                className="inline-flex items-center gap-1.5 rounded-lg border border-brand-200 dark:border-brand-500/20 bg-brand-100 dark:bg-brand-500/10 px-3 py-1.5 text-xs font-medium text-brand-600 dark:text-brand-400 hover:bg-brand-100 disabled:opacity-50">
        ...
        <div className="flex border-b border-zinc-200 dark:border-white/[0.07] px-6">
          {["Thông tin cơ bản", "Điều kiện cho vay", "Nguồn vốn & VĐƯ", "Hiệu quả & XH"].map((label, i) => (
            <button key={i} type="button" onClick={() => setActiveSubtab(i)}
              className={`cursor-pointer px-3 py-2.5 text-xs font-medium transition-colors duration-150 border-b-2 -mb-px ${activeSubtab === i ? "border-brand-500 text-brand-500 dark:text-brand-400" : "border-transparent text-zinc-500 dark:text-slate-400 hover:text-zinc-700 dark:hover:text-slate-300"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 px-6 py-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="cursor-pointer rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white transition-all duration-150 hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/40">
