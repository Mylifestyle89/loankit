...
export async function submitExtractedDocxImport(
  ...
  const collateralPromises = payload.collaterals
    .filter((col) => col.name)
    .map((col) => {
      // Type-specific `properties` bag — backend stringifies into properties_json.
      // Include all fields present on the extracted collateral; backend ignores
      // unknowns by virtue of stuffing them into a free-form JSON blob.
